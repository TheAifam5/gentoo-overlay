...
SRC_URI="
	...
	amd64? ( NVIDIA-Linux-x86_64-${NV_GRID_VER}-vgpu-kvm.run )
...
patch_file() {
	# Usage: patch_file FILE OFFSET REPLACEMENT
	# - FILE: path to the file
	# - OFFSET: byte offset (decimal, 0xHEX, or HEX)
	# - REPLACEMENT: hex bytes to write (spaces allowed)

	local file="$1"
    local offset_input="$2"
    local replacement="$3"

    # Normalize offset
    local offset
    if [[ "$offset_input" =~ ^0x[0-9A-Fa-f]+$ ]]; then
        offset=$((offset_input))
    elif [[ "$offset_input" =~ ^[0-9]+$ ]]; then
        offset=$offset_input
    elif [[ "$offset_input" =~ ^[0-9A-Fa-f]+$ ]]; then
        offset=$((16#$offset_input))
    else
        echo "Invalid offset format: $offset_input"
        return 1
    fi

    # Remove spaces from replacement
    local replacement_nospace=$(echo "$replacement" | tr -d ' ')
...
pkg_setup() {
	...
	local CONFIG_CHECK="
		...
		$(usev vgpu 'IOMMUFD_VFIO_CONTAINER VFIO_MDEV KVM_VFIO VFIO_PCI_CORE')
	"
...
src_unpack() {
	if use amd64; then
		local exclude="NVIDIA-Linux-x86_64-${NV_GRID_VER}-vgpu-kvm.run"

		# Unpack normal archives into ${WORKDIR}
		cd "${WORKDIR}" || die
		unpacker $(echo ${A} | tr ' ' '\n' | grep -v "^${exclude}$")

		# Unpack excluded one into different dir
		mkdir -p "${WORKDIR}/vgpu" || die
		cd "${WORKDIR}/vgpu" || die
		unpacker "${exclude}"
	else
		unpacker_src_unpack
...
src_prepare() {
	...
	if use vgpu; then
		mv vgpu/{README.vgpu,GRID_LICENSE,grid-third-party-licenses.txt,is_vgpu_host_package.txt,vgpuConfig.xml} . || die
		mv vgpu/{sriov-manage,nvidia-{xid-logd,vgpu{d,-mgr}}} . || die
		mv vgpu/libnvidia-vgpu.so.${NV_GRID_VER} libnvidia-vgpu.so.${PV} || die
		mv vgpu/libnvidia-vgxcfg.so.${NV_GRID_VER} libnvidia-vgxcfg.so.${PV} || die

		mkdir -p init-scripts/{systemd,sysv,upstart} || die

		mv vgpu/init-scripts/systemd/nvidia-vgpu-mgr.service init-scripts/systemd || die
		mv vgpu/init-scripts/systemd/nvidia-vgpud.service init-scripts/systemd || die
		mv vgpu/init-scripts/systemd/nvidia-xid-logd.service init-scripts/systemd || die

		mv vgpu/init-scripts/sysv/nvidia-vgpu-mgr init-scripts/sysv || die
		mv vgpu/init-scripts/sysv/nvidia-vgpud init-scripts/sysv || die
		mv vgpu/init-scripts/sysv/nvidia-xid-logd init-scripts/sysv || die

		mv vgpu/init-scripts/upstart/nvidia-vgpu-mgr.conf init-scripts/upstart || die
		mv vgpu/init-scripts/upstart/nvidia-vgpud.conf init-scripts/upstart || die

		mv vgpu/kernel/nvidia-vgpu-vfio kernel/nvidia-vgpu-vfio || die
		cp "${FILESDIR}"/vgpu.ld kernel/nvidia || die
		cp "${FILESDIR}"/vgpu_unlock_hooks.c kernel/common/inc/ || die
		sed -i '/"nv-time.h"/a #include "vgpu_unlock_hooks.c"' kernel/nvidia/os-interface.c || die
		sed -i '$ a ldflags-y += -T $(src)/nvidia/vgpu.ld' kernel/nvidia/nvidia.Kbuild || die

		echo 'NVIDIA_SOURCES += nvidia/nv-vgpu-vfio-interface.c' >> kernel/nvidia/nvidia-sources.Kbuild || die
		mv vgpu/kernel/nvidia/nv-vgpu-vfio-interface.c kernel/nvidia/nv-vgpu-vfio-interface.c || die
		mv vgpu/kernel/common/inc/nv-vgpu-vfio-interface.h kernel/common/inc/nv-vgpu-vfio-interface.h || die

		mv vgpu/kernel-open/nvidia-vgpu-vfio kernel-open/nvidia-vgpu-vfio || die
		cp "${FILESDIR}"/vgpu.ld kernel-open/nvidia || die
		cp "${FILESDIR}"/vgpu_unlock_hooks.c kernel-open/common/inc/ || die
		sed -i '/"nv-time.h"/a #include "vgpu_unlock_hooks.c"' kernel-open/nvidia/os-interface.c || die
		sed -i '$ a ldflags-y += -T $(src)/nvidia/vgpu.ld' kernel-open/nvidia/nvidia.Kbuild || die

		echo 'NVIDIA_SOURCES += nvidia/nv-vgpu-vfio-interface.c' >> kernel-open/nvidia/nvidia-sources.Kbuild || die
		mv vgpu/kernel-open/nvidia/nv-vgpu-vfio-interface.c kernel-open/nvidia/nv-vgpu-vfio-interface.c || die
		mv vgpu/kernel-open/common/inc/nv-vgpu-vfio-interface.h kernel-open/common/inc/nv-vgpu-vfio-interface.h || die

		# fix clang compilation
		sed -i 's/nv_free_vgpu_type_info()/nv_free_vgpu_type_info(void)/' kernel{,-open}/nvidia-vgpu-vfio/vgpu-ctldev.c || die

		# enable vGPU build
		sed -i '/# VGX_KVM_BUILD/a VGX_KVM_BUILD=1' kernel{,-open}/conftest.sh || die

		sed -i 's/nvidia-uvm  nvidia-modeset/nvidia-uvm nvidia-vgpu-vfio nvidia-modeset/' .manifest || die
		cat >> .manifest << EOF
...
kernel/nvidia/nv-vgpu-vfio-interface.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/common/inc/nv-vgpu-vfio-interface.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/nvidia-vgpu-vfio.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/nvidia-vgpu-vfio.Kbuild 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/nvidia-vgpu-vfio.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/vgpu-vfio-mdev.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/vgpu-devices.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/vgpu-ctldev.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/vgpu-devices.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/nv-vgpu-ioctl.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/nv-pci-table.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/nv-pci-table.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/nvidia-vgpu-vfio-sources.Kbuild 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/vgpu-vfio-pci-core.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/vgpu-egm.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel/nvidia-vgpu-vfio/vgpu-egm.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu

kernel-open/nvidia/nv-vgpu-vfio-interface.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/common/inc/nv-vgpu-vfio-interface.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/nvidia-vgpu-vfio.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/nvidia-vgpu-vfio.Kbuild 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/nvidia-vgpu-vfio.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/vgpu-vfio-mdev.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/vgpu-devices.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/vgpu-ctldev.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/vgpu-devices.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/nv-vgpu-ioctl.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/nv-pci-table.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/nv-pci-table.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/nvidia-vgpu-vfio-sources.Kbuild 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/vgpu-vfio-pci-core.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/vgpu-egm.c 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu
kernel-open/nvidia-vgpu-vfio/vgpu-egm.h 0644 KERNEL_MODULE_SRC INHERIT_PATH_DEPTH:1 MODULE:vgpu

libnvidia-vgpu.so.${PV} 0755 VGX_LIB NATIVE MODULE:vgpu
libnvidia-vgpu.so 0000 VGX_LIB_SYMLINK NATIVE libnvidia-vgpu.so.${PV} MODULE:vgpu
libnvidia-vgxcfg.so.${PV} 0755 VGX_LIB NATIVE MODULE:vgpu
libnvidia-vgxcfg.so 0000 VGX_LIB_SYMLINK NATIVE libnvidia-vgxcfg.so.${PV} MODULE:vgpu
sriov-manage 0755 EXPLICIT_PATH /usr/lib/nvidia MODULE:vgpu
nvidia-xid-logd 0755 UTILITY_BINARY MODULE:vgpu
init-scripts/systemd/nvidia-xid-logd.service 0444 EXPLICIT_PATH /usr/lib/nvidia/systemd MODULE:vgpu
init-scripts/sysv/nvidia-xid-logd 0755 EXPLICIT_PATH /usr/lib/nvidia/sysv MODULE:vgpu
nvidia-vgpud 0755 UTILITY_BINARY MODULE:vgpu
vgpuConfig.xml 0444 EXPLICIT_PATH /usr/share/nvidia/vgpu MODULE:vgpu
init-scripts/systemd/nvidia-vgpud.service 0444 EXPLICIT_PATH /usr/lib/nvidia/systemd MODULE:vgpu
init-scripts/sysv/nvidia-vgpud 0755 EXPLICIT_PATH /usr/lib/nvidia/sysv MODULE:vgpu
init-scripts/upstart/nvidia-vgpud.conf 0444 EXPLICIT_PATH /usr/lib/nvidia/upstart MODULE:vgpu
nvidia-vgpu-mgr 0755 UTILITY_BINARY MODULE:vgpu
init-scripts/systemd/nvidia-vgpu-mgr.service 0444 EXPLICIT_PATH /usr/lib/nvidia/systemd MODULE:vgpu
...
README.vgpu 0444 DOCUMENTATION NVIDIA_GLX-1.0/ MODULE:vgpu
EOF

		NV_GRID_VER_ESCAPED="${NV_GRID_VER//./\\.}"
		PV_ESCAPED="${PV//./\\.}"

		jq -n \
			--arg search "${NV_GRID_VER_ESCAPED}" --arg replace "${PV}" \
			'
				# Escape regex special characters from $search
				inputs | map(
					if .name | test($search)
						then .name |= sub($search; $replace)
						else .
					end
				) | unique_by(.name)
			' \
			sandboxutils-filelist.json \
			vgpu/sandboxutils-filelist.json \
			> sandboxutils-filelist-merged.json || die
		mv sandboxutils-filelist-merged.json sandboxutils-filelist.json || die

		if [ "${PV}" != "${PV_ESCAPED}" ]; then
...
src_compile() {
	...
	if use modules; then
		...
		use vgpu && modlist+=( nvidia-vgpu-vfio=${modlistargs} )
...
src_install() {
	...
	use vgpu && DOCS+=( README.vgpu GRID_LICENSE grid-third-party-licenses.txt )
...
documentation that is installed alongside this README."
	...
	if use vgpu; then
		systemd_dounit init-scripts/systemd/nvidia-{vgpu{d,-mgr},xid-logd}.service
	fi
	...
	while IFS=' ' read -ra m; do
		...
		if [[ -v 'paths[${m[2]}]' ]]; then
			...
			[[ ${m[0]} == init-scripts/* ]] && continue
			into=${m[3]}
			[[ ${m[0]} == sriov-manage ]] && into=/usr/bin
