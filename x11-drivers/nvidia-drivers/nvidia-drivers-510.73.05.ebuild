...
SRC_URI="
	amd64? (
		...
		vgpu? ( NVIDIA-Linux-x86_64-${VGPU_PV:=${PV}}-vgpu-kvm.run )
	)
...
SLOT="0/${PV%%.*}"
KEYWORDS="-* ~amd64"
IUSE="+X abi_x86_32 abi_x86_64 +driver persistenced static-libs +tools wayland vgpu"
...
BDEPEND="
	sys-devel/m4
	virtual/pkgconfig
	vgpu? ( dev-util/patchelf )"

QA_PREBUILT="lib/firmware/* usr/lib*"
...
src_unpack() {
	# remove vgpu-kvm.run from the extraction list
	CONSUMER_PKGS="$(echo $A | tr ' ' '\n' | grep -v "\-vgpu\-kvm\.run$")"
	unpacker ${CONSUMER_PKGS}
...
pkg_setup() {
	...
	use vgpu && MODULE_NAMES+=" nvidia-vgpu-vfio(video:kernel)"

	linux-mod_pkg_setup

	[[ ${MERGE_TYPE} == binary ]] && return

	BUILD_PARAMS='NV_KVM_MIGRATION_UAPI=1 NV_VERBOSE=1 IGNORE_CC_MISMATCH=yes SYSSRC="${KV_DIR}" SYSOUT="${KV_OUT_DIR}"'
...
src_prepare() {
	...
	if use vgpu; then
		...
		for i in nvidia-vgpud nvidia-vgpu-mgr nvidia-smi; do
			patchelf --add-needed libvgpu_unlock_rs.so "${i}" || die
		done
	fi
	...
	use !vgpu || sed -i 's/\(--ignore-remove\)/\1 nvidia-vgpu-vfio/' "${T}"/nvidia.conf || die
}
...
src_install() {
	...
	local skip_types=(
		...
		EXPLICIT_PATH # handled separately / unused
	)

	local DOCS=(
		README.txt NVIDIA_Changelog supported-gpus/supported-gpus.json
		nvidia-settings/doc/{FRAMELOCK,NV-CONTROL-API}.txt $(usev vgpu "GRID_LICENSE grid-third-party-licenses.txt")
	)
...
https://wiki.gentoo.org/wiki/NVIDIA/nvidia-drivers"
	...
	# MODULE:vgpu non-skipped extras
	if use vgpu; then
		insinto /usr/share/nvidia/vgpu
		doins vgpuConfig.xml

		for i in nvidia-vgpud nvidia-vgpu-mgr; do
			fowners :video "/usr/bin/${i}"
			fperms 4710 "/usr/bin/${i}"

			newconfd "${FILESDIR}/${i}.confd" "${i}"
			newinitd "${FILESDIR}/${i}.initd" "${i}"
			systemd_dounit "init-scripts/systemd/${i}.service"
		done
